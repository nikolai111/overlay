...
-(IBAction)btn_RegisterClicked:(id)sender
{
    ...
    UIAlertView *alert=[[UIAlertView alloc] initWithTitle:nil message:@"By clicking on Register you accept terms and conditions" delegate:self cancelButtonTitle:@"Accept" otherButtonTitles:@"Decline", nil];
    alert.tag=1001;
    [alert show];
...
- (IBAction)btn_checkmarkClicked:(id)sender {
    ...
    if ( _button_checkmark.selected==NO)
    {
     _button_checkmark.selected=YES;
    }
    else
    {
        _button_checkmark.selected=NO;
...
-(void)CallSignupWebservice
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/webservicesignup",ApplicationDelegate.Mainurl]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[url standardizedURL]];
    [request setHTTPMethod:@"POST"];
    NSString *postData = [NSString stringWithFormat:@"firstname=%@&lastname=%@&email=%@&password=%@&birthdate=%@&payment_type=%@&payment_id=%@&devicetoken=%@&signup=1",_txt_firstname.text,_txt_lastname.text,_txt_email.text,_txt_password.text,_txt_dob.text,str_payment_id,_txt_paymentid.text,Str_Token];
...
-(void)CompleteProcess
{
    ...
    if ([[dict_signup objectForKey:@"response"] objectForKey:@"success"])
    {
        ...
        ApplicationDelegate.Userid=[NSString stringWithFormat:@"%@",[[[dict_signup objectForKey:@"profile"] objectForKey:@"id"] objectAtIndex:0]];
...
//    else if(_button_checkmark.selected==NO)
//    {
//        [ApplicationDelegate ShowAlertOnTitle:nil Message:@"Please accept trems and conditions"];
//        return NO;
//
//    }
